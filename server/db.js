...
function syncReferralCredit(data, order) {
  ...
  if (paid && order.referral_credit_status !== 'earned' && rewardCents > 0) {
    account.credit_balance_cents = Number(account.credit_balance_cents || 0) + rewardCents;
    order.referral_credit_cents = rewardCents;
    order.referral_credit_status = 'earned';
    addLedgerEntry(data, account.id, rewardCents, 'referral_reward', { orderId: order.id, note: `Referral reward from HP-${order.id}` });
  } else if (!paid && order.referral_credit_status === 'earned') {
    const reversal = Number(order.referral_credit_cents || rewardCents);
    account.credit_balance_cents = Number(account.credit_balance_cents || 0) - reversal;
...
function deleteOrder(id) {
  ...
  if (order.referral_credit_status === 'earned') {
...
function referralStatsFromData(data, accountId, options = {}) {
  ...
  const recentReferrals = [...paidOrders]
    ...
    .slice(0, 12)
    .map(order => ({ orderId: order.id, customer: order.buyer && order.buyer.name ? String(order.buyer.name).split(/\s+/)[0] : 'Customer', productSpend: dollars(Math.max(0, cents(order.subtotal) - cents(order.discount_amount))), creditEarned: dollars(order.referral_credit_cents), paidAt: order.paid_at || order.created_at }));
  return {
    uniqueCustomers,
...
function getAccountDashboard(id, options = {}) {
  ...
  return { account, stats, orders, ledger, payouts };
}
...
module.exports = {
  ...
  createAccount, getAccountById, getAccountByEmail, setAccountVerificationToken, verifyAccountByTokenHash, setPasswordResetToken, resetPasswordByTokenHash, touchAccountLogin, getAccountByReferralCode, setAccountReferralCode, getAccountDashboard, createPayoutRequest, updatePayoutRequest, getAdminReferralData,
};
