...
3. **Admin password & session secret.** Set `ADMIN_PASSWORD_SHA256` (preferred) or `ADMIN_PASSWORD`, plus `SESSION_SECRET`, before deploying — do not leave the defaults in `config.js`. The restricted `/promo-manager.html` login stays disabled until the private `PROMO_MANAGER_USERNAME` and `PROMO_MANAGER_PASSWORD_SHA256` environment variables are set; it can only create fixed 15%-off codes. Its sign-ins and code activity appear only in the full admin dashboard's **Promo Audit** tab.
