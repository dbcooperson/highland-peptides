...
3. **Admin password & session secret.** Set `ADMIN_PASSWORD_SHA256` (preferred) or `ADMIN_PASSWORD`, plus `SESSION_SECRET`, before deploying — do not leave the defaults in `config.js`. The restricted `/promo-manager.html` login stays disabled until private promo credentials are configured. The legacy `PROMO_MANAGER_USERNAME` and `PROMO_MANAGER_PASSWORD_SHA256` variables configure one account; `PROMO_MANAGER_ACCOUNTS_JSON` can add more as a JSON array of `{ "username": "...", "passwordSha256": "..." }` records. Promo accounts can only create fixed 15%-off codes, and every sign-in and code action appears only in the full admin dashboard's **Promo Audit** tab.
