...
function renderDashboard(data) {
  ...
  renderActivity(document.getElementById('accountOrders'), orders || [], item => {
    const trackingText = item.trackingNumber ? `${item.trackingCarrier || 'Tracking'}${item.trackingService ? ` ${item.trackingService}` : ''} · ${item.trackingNumber}` : '';
    const delivery = item.trackingEstimatedDelivery ? `<span>Expected delivery: ${escapeHTML(item.trackingEstimatedDelivery)}</span>` : '';
